...
// Metric Names
pub const METRIC_HEARTBEAT: &str = "jokoway_heartbeat";
pub const METRIC_ACTIVE_CONNECTIONS: &str = "jokoway_active_connections";
pub const METRIC_ROUTER_DURATION: &str = "jokoway_router_duration_seconds";
pub const METRIC_TRANSFORMER_DURATION: &str = "jokoway_transformer_duration_seconds";
pub const METRIC_REQUESTS_TOTAL: &str = "jokoway_requests_total";
pub const METRIC_REQUEST_DURATION: &str = "jokoway_request_duration_seconds";
pub const METRIC_WS_EXTENSION_DURATION: &str = "jokoway_websocket_extension_duration_seconds";

pub struct MetricsExtension;

impl JokowayExtension for MetricsExtension {
    fn jokoway_init(&self, server: &mut Server, app_ctx: &mut AppCtx) -> Result<(), JokowayError> {
        ...
        if let Some(listen_addr) = &config.metrics_listen {
            ...
            backend
                .init_metrics()
                .map_err(|e| JokowayError::Other(format!("Failed to initialize metrics: {}", e)))?;

            app_ctx.insert(backend);
            let backend = app_ctx.get::<PrometheusBackend>().ok_or_else(|| {
                JokowayError::Other("Failed to retrieve PrometheusBackend from AppCtx".to_string())
            })?;
...
mod tests {
    ...
    fn test_metrics_extension_init() {
        ...
        let config = JokowayConfig {
            metrics_listen: Some("127.0.0.1:9091".to_string()),
            ..Default::default()
        };
        app_ctx.insert(config);

        let extension = MetricsExtension;
        extension.jokoway_init(&mut server, &mut app_ctx).unwrap();
